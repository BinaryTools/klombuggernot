...
import com.bt.menu.MainMenuEstudy;
...
public class Log_EstudianteFlux extends UnsecureBaseFlux {
...
//        ctrl.setUserDto(new SEG_UsuariSistemDto());
        crearLogin("Estudiantes: Acceso al Sistema");
        ctrl.init();
        add(ctrl);
        ctrl.add(new AttributeModifier("style", "width:30%;display:block"));
    }
    protected void gotoMainPage(AjaxRequestTarget art) {
        setResponsePage(MainMenuEstudy.class);
    }
    
    private void crearLogin(String title) {
        SNode node = new SNode(0, 0, title);
        TPreFormNode logNode = new TPreFormNode(SNode.NODE_ID, fields(), 1);
        node.add(logNode);
        ctrl.add(node);
    }
    ...
    private boolean aceptar(AjaxRequestTarget art){
        ...
        EstuAcceLogic lgc = new EstuAcceLogic(new Connect());
        
//        if (!lgc.getInvalidChars(passWord).equals("")){
//            message("Uso de caracreres invalidos",art);
//            //art.add();
//            return false;
//        }         
    ...
    private ArrayList fields() {
        ...
        flds.add(new GImage(0,0,"", logoUrl));
        
        flds.add(new GEditField(1, 0, new PropertyModel(this, "userName"), "Ingrese su codigo estudiantil:", true));
        flds.add(new GPassField(2, 0, new PropertyModel(this, "passWord"), "Ingrese su clave:"));
        flds.add(new TButton(5, 0, "Ingresar", "") {
            @Override
        ...
        flds.add(img = new GImage(3, 0, "", new PropertyModel(this,"captcha")));
        flds.add(imgTextFld=new GEditField(4, 0, new PropertyModel(this,"userText"), "Ingrese el texto de la imagen de arriba"));
        img.setVisible(false);
        imgTextFld.setVisible(false);
        flds.add(new GText(6, 0, new PropertyModel(this, "info"), "La U le dice:"));
        msg = new GText(7,0,new PropertyModel(this,"mensajes"),"background-color:#FDEFA7;color:black;");
        flds.add(msg);
        flds.add(new GText(8,0,"<center><a href='http://www.univex.com.co' target:_blank ><strong>Univex IV - Binary Tools &copy;<br/>2017</strong></a></center>"));
