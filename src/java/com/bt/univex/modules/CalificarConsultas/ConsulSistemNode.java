...
//                   if(consDto.getNotaConsSist() != 0){
//                       notaconssistFld.setEnabled(false);
//                   }else{
//                       notaconssistFld.setEnabled(true);
//                   }
...
//               coorestuvLku.setEnabled(false);
           ...
           // este es el método para el caso de tener que modificar algo a la hora de borrar
            @Override
            protected boolean deleteRecord(AjaxRequestTarget art, BaseDto dto) {
//                try {
//                    CJDVConsulSistemDto consSistVDto = (CJDVConsulSistemDto) dto;
//                    CJD_ConsulSistemDao consSistDao = new CJD_ConsulSistemDao();
//                    CJD_PersonConsulDao persConsDao = new CJD_PersonConsulDao();
//                    //borra wn la tabla de CJD_ConsulSitem y en la tabla de CJD_PersonConsulDao
//                    consSistDao.delete(consSistVDto.getPkeyConsSist());
//                    persConsDao.delete(consSistVDto.getPkeyPersCons());
...
//                }
           ...
           public boolean update(AjaxRequestTarget art, BaseDto inDto) {
               ...
               consSistDto.setFechApruCoor(consSistVDto.getFechApruCoor());
...
//---------------------------- CREAR GRID 
       ...
       if (pkeyDoceEspe != 0) {//si es diferente de cero muestra las consultas asociadas, de lo contrario no muestra ninguna consulta 
           cjdvconsulsistemProv.getDao().setConditions("fkeyDoceEspe = " + pkeyDoceEspe+" and EstaAproCons = 'S'"
                   + " and fkeygrupasig in"
...
//       cols.add(new ColText("TipoUsuaSist","Tipo de Usuario",120)); 
       ...
       cols.add(new ColText("FechApruCoor", "Aprobado el:", 200));
...
//       flds.add(locausuasistFld= new TEditField    (3,3,"LocaUsuaSist","Localidad",true));
       ...
       flds.add(valodoceconsFld = new TMemoField(14, 1, 3, 1, "ValoDoceCons", "Valoración del docente asesor", false){
            @Override
