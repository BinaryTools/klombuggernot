...
//---------------------------- CREAR GRID 
        CJD_ProcesConsulProv cjdvprocesconsulProv = new CJD_ProcesConsulProv();
...
//------------------------------------------------------------------------
    ...
    public void createFinder(int id, int indent, int parent, final ControlNode ctrl) {
        ...
        if (pkeyDoceEspe != 0) {//si es diferente de cero muestra los procesos asociadas, de lo contrario no muestra ninguna consulta 
            cjdvprocesconsulProv.getDao().setConditions("fkeyDoceEspe = " + pkeyDoceEspe + " and estaprocesos != 'F'");
        } else {
        ...
        gridNode.setTitle("Calificación procesos");
    ...
    public ArrayList<TColumn> genFieldsGrid() {
        ...
        cols.add(new ColReport("Reporte", 100) {
            public String onRegister(BaseDto dto) {
                CJD_ProcesConsulDao proConDao = new CJD_ProcesConsulDao();
                proConDao.setConditions("PkeyProcCons=" + dto.toInteger());
                CJD_ProcesConsulDto procConsDto = (CJD_ProcesConsulDto) proConDao.findFirst();
                ...
                if (procConsDto.getEstaProcesos().equals("F")) {//valida si el proceso ya finalizo 
                    ...
                    if (estProDto == null) {
                        ...
                        if (estProDto != null) {
                            fkEstu = estProDto.getFkeyEstudian();
                        } else {
                            fkEstu = 0;
                        }
                        //asigna la fkey del estudiante que tiene a cargo el procesoen estos momentos
                    } else {
...
//                return "";            
                if (pppDto == null) {
                    return "http://reportes.umng.edu.co:8080/openreports/executeReport.action?reportId=416794&userName=PlanXprc43&Inicio=" + dto.toInteger() + "&PkeyPeriPlan=" + 0 + "&password=uvxfhx21&displayInline=true&exportType=0";
        ...
        cols.add(new ColText("NombProcesos", "Nombre Proceso", 200) {
            ...
            public void showData(String id, WebMarkupContainer parent, Object obj) {
                CJDVProcesConsulDto procConsDto = (CJDVProcesConsulDto) getChild();
                super.showData(id, parent, obj);
                String info = obj.toString();
                if ("F".equals(procConsDto.getEstaProcesos())) {
        ...
        cols.add(new ColText("NumeCasoProc", "Número caso", 200) {
            ...
            public void showData(String id, WebMarkupContainer parent, Object obj) {
                ...
                super.showData(id, parent, obj);
                String info = obj.toString();
                if (procConsDto.getEstaProcesos() == "F") {
                    style = "background-color:#E6E6E6";
                } else {
                    style = "";
                }
...
//        cols.add(new ColText("FechRelaProc", "FechRelaProc", 120));
////        cols.add(new ColText("TipoUsuaSist", "Tipo Usuario", 120));
//        cols.add(new ColText("NombUsuaSist", "Nombre Usuario", 200));
//        cols.add(new ColText("TipoDocuUsua", "Tipo Documento", 120));
//        cols.add(new ColText("NumeDocuUsua", "Número Documento", 200));
//        cols.add(new ColText("OcupUsuaSist", "Ocupción", 200));
...
//        cols.add(new ColText("CorrUsuaSist", "Correo Electrónico", 200));
//        cols.add(new ColText("EstrUsuaSist", "Estrato", 200));
